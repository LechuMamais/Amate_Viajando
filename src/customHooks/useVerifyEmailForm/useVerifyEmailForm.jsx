...
const useVerifyEmailForm = () => {
    ...
    try {
      ...
      if (response.message === "Correo electrónico verificado correctamente") {
        ...
        toast({
          ...
          description:
            "Introduce correctamente el código que te hemos enviado al correo proporcionado",
          status: "error",
          duration: 5000,
          isClosable: true,
        });
  ...
  const generateAndSendNewVerificationToken = async () => {
    try {
      ...
      if (
        response.message ===
        "Hemos generado y enviado un nuevo código de verificación"
      ) {
        toast({
          title: "Hemos enviado un nuevo código de verificación",
          description:
            "Introduce el nuevo código para verificar tu correo electrónico",
          status: "success",
          duration: 5000,
          isClosable: true,
        });
